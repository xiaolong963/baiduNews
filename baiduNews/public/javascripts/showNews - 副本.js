...
window.onresize = window.onscroll = window.onload = function(){
	...
	var scrolHeight = $(window).scrollTop();//滚动高度
	$('.images').each(function(index,ele){
		if(winHeight+scrolHeight>$(ele).offset().top){
...
$(function(){
	...
	//new IScroll('#nav', { eventPassthrough: true, scrollX: true, scrollY: false, preventDefault: false });
